...
class siteController extends Controller
{
    ...
    public function listbuildings(Request $req)
    {
        ...
        if(!empty($req->listby)){
            ...
            foreach ($buildIds as $valId) {
                ...
                $get_list = clientModel::select('clnt_id', 'c_name', 'mobile', 'building_id', 'b_name')->where('building_id', $tId)->where('assign', $listby)->get();
    ...
    public function save_leaverequest(Request $req)
    {
        if(!empty($req->reason) && !empty($req->start_date)){
            ...
            $startDate = date('d-m-Y', strtotime($req->start_date));
			
            $endDate = !empty($req->end_date)?date('d-m-Y', strtotime($req->end_date)):$startDate;
            ...
            if(strtotime($endDate) < strtotime($currentDate)){
                ...
                return response()->json([
                    'error'=>true,
                    'code'=>201,
                    'message'=>'Please Select a End Date After '. $currentDate .' !',
                ]);
            ...
            $getSecDtl = securityModel::select('supervised_by')->get();
    ...
    public function listleaverequest(Request $req)
    {
        ...
        if(isset($req->listby)){
            ...
            $get_list = leaverequestsModel::select('id as request_id', 'user_id','reason','approved','start_date','end_date')->where('approved_by', $user_id)->where('approved', $listby)->orderBy('id', 'desc')->get();
    ...
    public function listsecurityworkdtls(Request $req){
        ...
        if($user_type == 'SPVR'){
            ...
            $updCLntDtl = clientModel::where('clnt_id','=',$user_id)->get();
    ...
    public function save_worklocations(Request $req)
    {
        ...
        if(!empty($req->latitude) && !empty($req->longitude)){
            ...
            if($insLoc > 0){
                ...
                $getlatlng = clientModel::select('bl_lat', 'bl_lng')->where('building_id', $getCPID->building_id)->first();
                ...
                if( $fm > 0.1 ){
                    ...
                    $getSfcmId = supervisorModel::select('name','fcm_id')->where('spvr_id','=',$getCPID->supervised_by)->get();
